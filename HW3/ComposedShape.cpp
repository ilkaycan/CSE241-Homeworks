...
void ComposedShape::draw(ofstream &myfile) const{
    ...
    if ( shapeSmall == Shape::Rectangle ){
        for (auto i = 0; i < InnerShape.size() ; ++i){
            myfile << "<rect x=\""<< static_cast<Rectangle*>(InnerShape[i].SmallShape)->getX() <<"\" y=\"" << static_cast<Rectangle*>(InnerShape[i].SmallShape)->getY() << "\" width=\"" << static_cast<Rectangle*>(InnerShape[i].SmallShape)->getSide1();
            myfile <<"\" height=\""<< static_cast<Rectangle*>(InnerShape[i].SmallShape)->getSide2() << "\" stroke=\"black\" stroke-width=\"1\" fill=\"purple\" />" << endl;
        }
    } else if ( shapeSmall == Shape::Triangle){
        for(auto j = 0; j < InnerShape.size(); ++j){
            myfile << "<polygon points=\"" << static_cast<Triangle*>(InnerShape[j].SmallShape)->getX1() << "," << static_cast<Triangle*>(InnerShape[j].SmallShape)->getY1() << " " ;
            myfile << static_cast<Triangle*>(InnerShape[j].SmallShape)->getX2() <<"," << static_cast<Triangle*>(InnerShape[j].SmallShape)->getY2() << " " ;
            myfile << static_cast<Triangle*>(InnerShape[j].SmallShape)->getX3() << "," << static_cast<Triangle*>(InnerShape[j].SmallShape)->getY3() << "\"" << endl;
            myfile << "stroke=\"black\" stroke-width=\"1\" fill=\"purple\" />" << endl;
        }
    } else if ( shapeSmall == Shape::Circle){
        for(auto k = 0; k < InnerShape.size(); ++k){
            myfile << "<circle cx=\""<< static_cast<Circle*>(InnerShape[k].SmallShape)->getCx() <<"\" cy=\"" << static_cast<Circle*>(InnerShape[k].SmallShape)->getCy() ;
            myfile << "\" r=\"" << static_cast<Circle*>(InnerShape[k].SmallShape)->getRadius();
            myfile << "\" stroke=\"black\" stroke-width=\"1\" fill=\"purple\" />" << endl;
        }
    } else {
        cerr << "Something went wrong :(" << endl;
...
// decide if small shape is horizontal or vertical
    ...
    for ( int i = (int)(ContainB/SmallB) ; i>0; i--){
        for(int j = (int)(ContainA/SmallA) ; j>0 ; j --){
            ...
            InnerShape.push_back(ShapeElem(rect));
            x += SmallA;
        }
        x = 0;
        y += SmallB;
...
inline void ComposedShape::RToptimal(){
    ...
    for ( int i = (int)(ContainB/(SQRT3*(SmallA/2))) ; i>0; i--){
        for(int j = (int)(ContainA/(SmallA)) ; j>0 ; j--){
            ...
            InnerShape.push_back(ShapeElem(tri));
            xup += SmallA;
            xright += SmallA;
            xleft += SmallA;
        }
    ...
    for ( int i = (int)(ContainB/(SQRT3*(SmallA/2))) ; i>0; i--){
        for(int j = (int)((ContainA-(SmallA/2))/(SmallA)) ; j>0 ; j --){
            ...
            InnerShape.push_back(ShapeElem(tri2));
            adown += SmallA;
            aright += SmallA;
            aleft += SmallA;
        }
...
inline void ComposedShape::RCoptimal(){
    ...
    for ( int i = (int)(ContainB/(SmallA*2)); i>0; i--){
        for(int j = (int)(ContainA/(2*SmallA)) ; j>0 ; j --){
            ...
            InnerShape.push_back(ShapeElem(circ));
            cx += 2*SmallA;
        }
...
inline void ComposedShape::TCoptimal(){
    ...
    for ( int i = (int)((ContainA/(2*SmallA))-SQRT3+1) ; i>0; i--){
        for( int j =rownum ; j>0 ; j --) {
            ...
            InnerShape.push_back(ShapeElem(circle));
            cx += 2 * SmallA;
        }
...
ComposedShape::ShapeElem::ShapeElem(Rectangle &rect){
    ...
    SmallShape = new Rectangle(rect);
}
ComposedShape::ShapeElem::ShapeElem(Triangle &tri){
    shape = Shape::Triangle;
    SmallShape = new Triangle(tri);

}
ComposedShape::ShapeElem::ShapeElem(Circle &circ){
    shape = Shape::Circle;
    SmallShape = new Circle(circ);
}
//This function returns area of the shape ;
// which is pointed by ShapeElem's void pointer
double ComposedShape::ShapeElem::getareaElem(void){
    if(shape == Shape::Rectangle){
        return static_cast<Rectangle*>(SmallShape)->getArea();
    } else if(shape == Shape::Triangle){
        return static_cast<Triangle*>(SmallShape)->getArea();
    } else if(shape == Shape::Circle){
        return static_cast<Circle*>(SmallShape)->getArea();
    }
}
//This function returns perimeter length of the shape ;
// which is pointed by ShapeElem's void pointer
double ComposedShape::ShapeElem::getperimeterElem(void){
    if(shape == Shape::Rectangle){
        return static_cast<Rectangle*>(SmallShape)->getPerimeter();
    } else if(shape == Shape::Triangle){
        return static_cast<Triangle*>(SmallShape)->getPerimeter();
    } else if(shape == Shape::Circle){
        return static_cast<Circle*>(SmallShape)->getPerimeter();
    }
}

//This function sends outputstream to << overloading function in main for ShapeElem
ostream& ComposedShape::ShapeElem::output(ostream& outputStream)const{
    if(shape == Shape::Rectangle){
        return static_cast<Rectangle*>(SmallShape)->output(outputStream);
    }else if(shape == Shape::Triangle){
        return static_cast<Triangle*>(SmallShape)->output(outputStream);
    } else if(shape == Shape::Circle){
        return static_cast<Circle*>(SmallShape)->output(outputStream);
    }
    return outputStream;
}
...
//takes rectangle as a parameter and returns ShapeElem object
ComposedShape::ShapeElem ComposedShape::operator [](Rectangle &rect){
    ShapeElem temp(rect);
    return temp;
}
//This function overloads [] for ComposedShape Class
//takes triangle as a parameter and returns ShapeElem object
ComposedShape::ShapeElem ComposedShape::operator [](Triangle &tri){
    ShapeElem temp(tri);
    return temp;
}
//This function overloads [] for ComposedShape Class
//takes circle as a parameter and returns ShapeElem object
ComposedShape::ShapeElem ComposedShape::operator [](Circle &circ){
    ShapeElem temp(circ);
    return temp;
}
//This function overloads +- for ComposedShape Class
